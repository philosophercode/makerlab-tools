...
import { ADMIN_GROUPS, ADMIN_SURFACES, COUNT_LOADERS, SURFACE_KEYS, currentHref, surfacesFor } from "./surfaces";
...
describe("the list itself", () => {
  ...
  it.each(["anonymous", "user", "admin", "super_admin"] as const)(
    ...
    (role) => {
      ...
      // refusal. (Import a list gates on `tools.add`, which is not itself a
      // layout permission; every role holding it also holds one that is.)
...
describe("currentHref — the most specific surface a path is on", () => {
  ...
  it("marks Import a list, not also Intake, on the import page", () => {
    expect(currentHref("/admin/intake/imports/new", hrefs)).toBe("/admin/intake/imports/new");
  });

  it("marks Intake on the queue, an item and an import's review", () => {
