 ...
 * **one line, capped, and nothing that could close its fence**. Angle brackets
 * and backticks are dropped (the prompt fences it in a tag), control
 * characters too, and whitespace collapses to single spaces.
 *
 * Client-safe and plain Node: the route, the server action, the prompt and the
 * textarea all use the same rule.
 */

/** The note as one clean line (possibly empty). Does not cap: {@link parseReviewerNote} decides what is too long. */
